...
uint8_t Blockchain::getBlockMajorVersionForHeight(uint32_t height) const {
  if (height < HARDFORK_HEIGHT_2) {
    return BLOCK_MAJOR_VERSION_1;
  } else if (height < HARDFORK_HEIGHT_3) {
...
bool Blockchain::check_tx_input(const KeyInput& txin, const Crypto::Hash& tx_prefix_hash, const std::vector<Crypto::Signature>& sig, uint32_t* pmax_related_block_height) {
  ...
  if (m_blocks.size() > 1583230 && !(scalarmultKey(txin.keyImage, L) == I)) {
    return false;
  }
...
bool Blockchain::update_next_comulative_size_limit() {
  size_t blockGrantedFullRewardZone =
    getBlockMajorVersionForHeight(getCurrentBlockchainHeight()) < HARDFORK_HEIGHT_3 ?
...
bool Blockchain::pushBlock(const Block& blockData, const std::vector<Transaction>& transactions, block_verification_context& bvc) {
  ...
  const uint8_t expectedBlockVersion = getBlockMajorVersionForHeight(getCurrentBlockchainHeight());
  if (expectedBlockVersion != blockData.majorVersion) {
    logger(INFO, BRIGHT_WHITE) << "Block with id: " << blockHash << '\n'
      << "have wrong major version: " << static_cast<int>(blockData.majorVersion) << '\n'
      << "expected major version: " << static_cast<int>(expectedBlockVersion);
    bvc.m_verifivation_failed = true;
    return false;
  }
